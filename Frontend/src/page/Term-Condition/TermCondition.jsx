...
import { usePolicyAllQuery, usePolicyDeleteMutation, useTermAllQuery } from '../../rtk/policy';
import { isConfirm } from '../../helper/SweetAlrertIsConfirm';
import { ClipLoader } from 'react-spinners';

export default function TermCondition() {
  const { data: policies, isLoading, refetch } = useTermAllQuery();
  ...
  const addNewPolicy = () => {
    navigate("/dashboard/add/term-condition");
  };

  // edit
  const editPolicy=(policy)=>{
        navigate("/dashboard/add/term-condition", { state: { policyEdit: policy } });
  }
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Term & Condition</h1>
